...
//----------------------------------------------//
vector<game> readGameAtRound(string line, int typeInput, bool writeToFile, int* round, vector<team> teamsPtr, league* leaguePtr) {	
	vector<game> v;
	// type 1 from menu
	if (typeInput==1){
		...
		if (&gameTempDetails != NULL){
			...
			if (gameTempDetails.getRoundNum()<= *round+1){
				writeToGamesDB(line);
				...
				cout<<"error: you try to insert a game round muche higher. \nlast round was "<<*round<<" (change in the code to return null)"<<endl;
			}
		}
			...
			while(fileReader.good() &&	getline(fileReader,tmp) && splitStr(tmp).at(0)!= ";") {			//delete all spaces in tmp
				if (fileReader!=NULL && tmp.find("session",0) == std::string::npos){
					...
					if (tmp.find("game",0) != std::string::npos ){
						...
						if ( *round < gameTempDetails.getRoundNum())
							*round = gameTempDetails.getRoundNum();
					}
					//read specific game
